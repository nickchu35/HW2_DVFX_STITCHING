...
function [cim, r, c] = harris(im, sigma, thresh, radius, disp)
    ...
    Ix = conv2(im, dx, 'same');    % conv of im and dx --> Image derivatives
    Iy = conv2(im, dy, 'same');    
    ...
    if nargin > 2   % perform nonmaximal suppression and threshold
        % Extract local maxima by performing a grey scale morphological dilation and then finding points in the corner strength image that
